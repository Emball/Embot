...
REMOTE DEBUG EXEC IS READ-ONLY — NEVER use `exec` to edit code on the server. The server runs the live git repo; editing files directly with sed/echo/etc creates uncommitted changes that block `git pull --ff-only` on the next update, causing sync failures. All code edits must be made in the local repo, committed, and pushed — then the server pulls the clean commit. Use `exec` only for reading files, checking logs, or running read-only commands. Exceptions require explicit user approval.

### Claude Bridge Mode

The bot includes a second debug channel for Claude sessions — a GitHub-based command queue via the private `Emball/EmbotDebug` repo. Enabled via `config/remote_debug.json`:
...
When enabled, the bot polls `cmd.json` in EmbotDebug every `poll_interval` seconds via GitHub API. On a new `seq`, it executes the command internally and commits results back:

- **Direct output commands** (ping, status, guilds, modules, exec, update, restart) → `result.json`
- **File artifact commands** (logs, logs-list, logs-search, config, db-query) → committed as files under `logs/`, `config/`, `db/` in EmbotDebug
- **Blocked** (db-download, stream) — not supported via bridge
