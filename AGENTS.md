...
The token is auto-generated by the server on first startup and stored in the same config file. The client reads both `url` and `token` from `config/remote_debug.json` — no separate config needed.
