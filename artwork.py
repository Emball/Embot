...
async def handle_artwork_submission(bot, message):
    has_attachment = bool(message.attachments)
    has_link = re.search(r"https?://\S+", message.content) is not None
    is_valid = has_attachment or has_link

    if is_valid:
        image_extensions = r"\.(png|jpg|jpeg|gif|webp)(\?|$)"
        has_image_attachment = any(
            att.content_type and "image" in att.content_type
            for att in message.attachments
        )
        has_image_link = re.search(
            fr"https?://\S+{image_extensions}",
            message.content,
            re.IGNORECASE
        ) is not None
        is_valid = has_image_attachment or has_image_link
    ...
    if is_valid:
        ...
        try:
            ...
            warning_msg = (
                f"Your message in #artwork was removed because it didn't meet submission requirements.\n"
                f"Please include a valid image link/attachment.\n"
            ...
            print(f"Error handling invalid artwork submission: {e}")

async def handle_artwork_reaction(bot, payload):
    channel = await bot.fetch_channel(payload.channel_id)
    if channel.name != ARTWORK_CHANNEL_NAME:
        return
    ...
    bot.level_system.reaction_tracker.add(reaction_key)  # Now works with set()
