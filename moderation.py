...
def setup(bot):
    ...
    # ==================== HELPERS FOR PREFIX COMMANDS ====================

    async def send_tracked_response_ctx(ctx: commands.Context, embed: discord.Embed) -> int:
        """Send response and return message ID for tracking (Context version)"""
        try:
    ...
    async def log_mod_action_with_tracking_ctx(bot, ctx: commands.Context, action: str, user_id: Optional[int],
                                          user: Optional[str], reason: str, inchat_msg_id: Optional[int],
                                          botlog_msg_id: Optional[int], duration: Optional[str] = None,
                                          additional: Optional[dict] = None) -> Optional[str]:
        """Log moderation action to oversight system and track embeds (Context version)"""
        if not hasattr(bot, 'mod_oversight'):
            return None
    ...
    async def ban_prefix(ctx, user: discord.User, *, reason: str):
        """Ban a user"""
        
        # Check for elevated role
        if not has_elevated_role(ctx.author):
            await ctx.send(ERROR_NO_PERMISSION)
            return
        ...
        # Self-check
        if user == ctx.author:
            await ctx.send(ERROR_CANNOT_ACTION_SELF)
            return
        
        # Bot check
        if user == bot.user:
            await ctx.send(ERROR_CANNOT_ACTION_BOT)
            return
        ...
        if member:
            if member.top_role >= ctx.author.top_role and ctx.author != ctx.guild.owner:
                await ctx.send(ERROR_HIGHER_ROLE)
                return
        ...
        try:
            ...
            inchat_embed.add_field(name="Reason", value=reason, inline=False)
            inchat_embed.add_field(name="Moderator", value=ctx.author.mention, inline=True)
            inchat_embed.add_field(name="Messages Deleted", value=f"{delete_days} days", inline=True)
            
            inchat_msg_id = await send_tracked_response_ctx(ctx, inchat_embed)
            ...
            # Log to oversight
            await log_mod_action_with_tracking_ctx(
                bot=bot,
                ctx=ctx,
                action='ban',
                user_id=user.id,
    ...
    async def unban_prefix(ctx, user_id: str, *, reason: str = "No reason provided"):
        ...
        try:
            ...
            bot.logger.log(MODULE_NAME, f"{ctx.author} unbanned {user}")
    ...
    async def kick_prefix(ctx, member: discord.Member, *, reason: str):
        """Kick a member"""
    ...
    async def timeout_prefix(ctx, member: discord.Member, duration: int, *, reason: str):
        """Timeout a member"""
        ...
        try:
            ...
            botlog_msg_id = await _mod_log(bot).log_timeout(
                ctx.guild, member, ctx.author, reason,
            ...
            bot.logger.log(MODULE_NAME, f"{ctx.author} timed out {member} for {duration} minutes")
    ...
    async def untimeout_prefix(ctx, member: discord.Member):
        ...
        if not ctx.author.guild_permissions.moderate_members:
            await ctx.send("❌ You don't have permission to moderate members.")
            return
        ...
        try:
            ...
            await ctx.send(embed=embed)
            bot.logger.log(MODULE_NAME, f"{ctx.author} removed timeout from {member}")
            
        except Exception as e:
            await ctx.send("❌ An error occurred while trying to remove the timeout.")
            bot.logger.error(MODULE_NAME, "Untimeout command failed", e)

    @bot.command(name="mute")
    async def mute_prefix(ctx, member: discord.Member, duration: Optional[str] = None, *, reason: str = "No reason provided"):
        """Mute a member"""
        # Note: Argument order flipped slightly to allow optional duration before reason if parsed, 
        # but with greedy reason, it's safer to do: ?mute @user 10m reason or ?mute @user reason
        # Since duration is optional, if the second arg isn't a duration, it's part of the reason?
        # Standard discord.py greedy behavior consumes rest. 
        # For simplicity in this prefix impl: ?mute @user [duration] [reason]
        # If duration is not provided, it's permanent.
        
        # Re-check duration parsing manually if needed, or rely on type hint.
        # If user types "?mute @user bad behavior", duration might take "bad".
        # We'll use a simple check.
        
        actual_reason = reason
        actual_duration = duration
        
        # Heuristic: if duration doesn't look like a duration, treat it as start of reason
        if duration and not re.match(r'^(\d+)([smhd])$', duration.lower()):
            actual_reason = f"{duration} {reason}" if reason else duration
        ...
        if actual_duration:
            actual_duration = actual_duration.lower()
            match = re.match(r'^(\d+)([smhd])$', actual_duration)
            if match:
                value, unit = int(match.group(1)), match.group(2)
                if unit == 's':
                    duration_seconds = value
        ...
        try:
            ...
            await member.add_roles(muted_role, reason=actual_reason)
            moderation_manager.mute_manager.add_mute(ctx.guild.id, member.id, actual_reason, ctx.author, duration_seconds)
            ...
            try:
                ...
                dm_embed.add_field(name="Reason", value=actual_reason, inline=False)
                dm_embed.add_field(name="Duration", value=duration_str, inline=True)
                dm_embed.add_field(name="Moderator", value=str(ctx.author), inline=True)
                await member.send(embed=dm_embed)
            except discord.Forbidden:
            ...
            inchat_embed.add_field(name="Reason", value=actual_reason, inline=False)
            inchat_embed.add_field(name="Duration", value=duration_str, inline=True)
            inchat_embed.add_field(name="Moderator", value=ctx.author.mention, inline=True)
            ...
            botlog_msg_id = await _mod_log(bot).log_mute(
                ctx.guild, member, ctx.author, actual_reason, duration_str, ctx.channel
            ) if _mod_log(bot) else None
    ...
    async def softban_prefix(ctx, member: discord.Member, *, reason: str):
        """Softban a member"""
        ...
        try:
            # Ban then unban
            await member.ban(reason=f"Softban: {reason} - By {ctx.author}", delete_message_days=delete_days)
            ...
            bot.logger.log(MODULE_NAME, f"{ctx.author} softbanned {member}")
            
        except Exception as e:
            try:
                await ctx.send("❌ An error occurred while trying to softban the member.")
            except:
                pass
            bot.logger.error(MODULE_NAME, "Softban command failed", e)

    @bot.command(name="purge")
    async def purge_prefix(ctx, amount: int, user: Optional[discord.Member] = None):
        """Purge messages"""
        ...
        async with ctx.typing():
            try:
                ...
                inchat_embed.add_field(name="Moderator", value=ctx.author.mention, inline=True)
                inchat_embed.add_field(name="Channel", value=ctx.channel.mention, inline=True)
                
                await ctx.send(embed=inchat_embed, delete_after=10)
                
                botlog_msg_id = await _mod_log(bot).log_purge(
                    ctx.guild, ctx.author, len(deleted), ctx.channel, user
                ) if _mod_log(bot) else None
                ...
                bot.logger.log(MODULE_NAME, f"{ctx.author} purged {len(deleted)} messages in {ctx.channel}")
    ...
    async def slowmode_prefix(ctx, seconds: int, channel: Optional[discord.TextChannel] = None):
        """Set slowmode"""
        ...
        try:
            ...
            await ctx.send(embed=embed)
            bot.logger.log(MODULE_NAME, f"{ctx.author} set slowmode to {seconds}s in {target_channel.name}")
    ...
    async def warn_prefix(ctx, member: discord.Member, *, reason: str):
        ...
        try:
            ...
            bot.logger.log(MODULE_NAME, f"{ctx.author} warned {member} (Total warnings: {strike_count})")
    ...
    async def clearwarnings_prefix(ctx, member: discord.Member):
        ...
        if not ctx.author.guild_permissions.administrator:
            await ctx.send("❌ You need Administrator permission to clear warnings.")
            return
        ...
        if success:
            ...
            await ctx.send(embed=embed)
            bot.logger.log(MODULE_NAME, f"{ctx.author} cleared warnings for {member}")
        else:
            await ctx.send(f"**{member}** has no warnings to clear.")

    @bot.command(name="lock")
    async def lock_prefix(ctx, *, reason: str):
        """Lock a channel"""
        # Prefix syntax: ?lock reason (current channel) or ?lock #channel reason?
        # Slash syntax: /lock reason channel
        # We'll support ?lock <reason> for current channel. 
        # For remote channel: ?lock <channel> <reason>
        # To handle both, we check if first word is a channel mention or ID.
        ...
        try:
            # Try to convert first arg to channel
            converter = commands.TextChannelConverter()
            possible_channel = await converter.convert(ctx, first_arg)
            ...
            # Not a channel, so the whole thing is a reason
            pass
        ...
        # Validate reason
        valid, error_msg = await validate_reason(actual_reason)
        if not valid:
            await ctx.send(error_msg)
            return
        ...
        try:
            ...
            await target_channel.set_permissions(
                ...
                send_messages=False,
                reason=f"{actual_reason} - By {ctx.author}"
            )
            ...
            botlog_msg_id = await _mod_log(bot).log_lock(
                ctx.guild, ctx.author, actual_reason, target_channel
            ) if _mod_log(bot) else None
            ...
            bot.logger.log(MODULE_NAME, f"{ctx.author} locked {target_channel.name}")
    ...
    async def unlock_prefix(ctx, channel: Optional[discord.TextChannel] = None):
        """Unlock a channel"""
        ...
        try:
            ...
            await ctx.send(embed=embed)
            bot.logger.log(MODULE_NAME, f"{ctx.author} unlocked {target_channel.name}")
