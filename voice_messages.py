...
class VoiceMessageManager:
    def __init__(self):
        ...
        # Tracking for improved randomness
        self.recent_drops = deque(maxlen=20)  # Keep last 20 drops
        self.file_usage = {}  # Track usage count per file
        
        # Initialize file tracking
        self._initialize_file_tracking()

    def _initialize_file_tracking(self):
        """Initialize file usage tracking from existing files"""
        for file in self.get_current_month_files():
            self.file_usage[str(file)] = 0

    def get_current_month_files(self):
        """Get all voice files from current month directory"""
        return list(self.current_month_dir.glob("*.ogg"))

    def get_archive_files(self):
        """Get all archived voice files"""
        archive_files = []
    ...
    def _archive_old_messages(self):
        """Move files from current month to archive at month change"""
        now = datetime.now()
        current_month = now.month
        current_year = now.year
        
        # Check if we've already archived this month
        if (self.archive_dir / str(current_year) / f"{current_month:02d}").exists():
            return
        
        # Create archive directory for previous month
        ...
        for file in self.get_current_month_files():
            try:
                ...
                print(f"Error archiving file {file}: {e}")
                ...
                for month_dir in year_dir.glob("*"):
                    ...
                    if dir_date < cutoff_date:
                        try:
                            ...
                            print(f"Error pruning archive {month_dir}: {e}")
            except ValueError:
                continue
    ...
    def _get_weighted_file_list(self):
        ...
        # Create weighted list (90% current, 10% archive)
        all_files = []
        weights = []
        ...
        if total_weight > 0:
            ...
            normalized_weights = None  # Will fall back to uniform
    ...
    async def save_voice_message(self, attachment):
        """Save a new voice message to current month directory"""
        timestamp = int(datetime.utcnow().timestamp() * 1000)
        filename = self.current_month_dir / f"voice-message-{timestamp}.ogg"
        try:
            await attachment.save(filename)
            self.file_usage[str(filename)] = 0  # Add to tracking
            return True
        except Exception as e:
        ...
        if file := voice_manager.get_random_voice_message():
            if file.exists():
                try:
                    ...
                    print(f"Error sending voice drop: {e}")